...
(function(exports) {
  ...
  var Html5AudioBuilder = Builder.extend({
    ...
    toHtml: function(cb) {
      ...
      if(this.sources && this.sources.length) {
        ...
        html || (html = '<a href="' + this.sources[0] +
  ...
  var Html5Strategy = Strategy.extend({
     ...
     * @inherits Builder
     */
    execute: function(mediaObject, options) {console.log(mediaObject['@type'])
      var builder;
      var media = getMedia(mediaObject) || {};
      ...
      if(media.playerType === 'iframe') {
        ...
        builder = new ScriptBuilder(options);
      } else if(!media['playerType'] && media.contentURL) {
        var poster = null;
      ...
      } else if(media['@type'] === 'MusicRecording' && media.audio
          && media.audio.contentURL) {
        extend(options, {
          sources: [media.audio.contentURL]
        });
